...
// BasicLog_Test.h,v 1.3 2000/05/16 22:09:32 pradeep Exp
...
private:
  ...
  int
    init_factory (CORBA::Environment &ACE_TRY_ENV);

  int
    parse_args();
  // command line argument parser

  int write_records(CORBA::ULongLong numberOfRecordsToWrite, CORBA::Environment &ACE_TRY_ENV);
  // writes enough records to fill the log. Default write enough to fill log. Hopefully, not infinite.

  void
    resolve_basic_factory (CORBA::Environment &ACE_TRY_ENV);
  // resolve log factory

  void
    resolve_naming_service (CORBA::Environment &ACE_TRY_ENV);
