...
void PropertyTableModel::createCell( const uint32& row, const uint32& column, TableCellItem** createdCell )
{
	if ( column == 1 ){
		*createdCell = new PropertyCellItem();
	}
	else {
		DefaultTableModel::createCell( row, column, createdCell );
	}
}
