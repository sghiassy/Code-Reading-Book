...
typedef struct _ilbmVisuals {
	struct _ilbmVisuals	*next;
	int						depth;
	int						bitsPerRGB;
	int						visuals;
	int						count;
} ilbmVisualsRec, *ilbmVisualsPtr;

static int  ilbmVisualPriority[] = {
	PseudoColor, DirectColor, GrayScale, StaticColor, TrueColor, StaticGray
};

#define NUM_PRIORITY		6

static ilbmVisualsPtr		ilbmVisuals;
...
ilbmSetVisualTypes(depth, visuals, bitsPerRGB)
	...
	int bitsPerRGB;
{
	ilbmVisualsPtr new, *prev, v;
	int count;

	new = (ilbmVisualsPtr)xalloc(sizeof *new);
	if (!new)
		return(FALSE);
	new->next = 0;
	new->depth = depth;
	...
	new->count = count;
	for (prev = &ilbmVisuals; v = *prev; prev = &v->next);
	*prev = new;
	return(TRUE);
}
	...
	int bitsPerRGB;
{
	...
	ilbmVisualsPtr visuals, nextVisuals;
	...
	if (!ilbmVisuals)
		for (f = 0; f < screenInfo.numPixmapFormats; f++) {
			...
			if (sizes & (1 << (b - 1)))
				...
				vtype = 0;
			if (!ilbmSetVisualTypes(d, vtype, bitsPerRGB))
				return(FALSE);
		}
	nvisual = 0;
	ndepth = 0;
	for (visuals = ilbmVisuals; visuals; visuals = nextVisuals) {
	...
	*nvisualp = nvisual;
	for (visuals = ilbmVisuals; visuals; visuals = nextVisuals) {
		...
		for (i = 0; i < NUM_PRIORITY; i++) {
			if (! (vtype & (1 << ilbmVisualPriority[i])))
				continue;
			visual->class = ilbmVisualPriority[i];
